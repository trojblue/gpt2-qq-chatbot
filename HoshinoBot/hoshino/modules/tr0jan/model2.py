...
# 允许发言, 不包含测试群
ALLOWED_GROUP = [863688150, 655057127, 1058878835]  #momo, mirai, mirai2
# 发言几率
RESPONSE_PROB = 0.3
# API
...
def is_valid_for_response(ev: CQEvent) -> bool:
    ...
    if ev.group_id in TEST_GROUP:  # 测试群
        ...
        if len(ev.message.extract_plain_text()) not in range (5, 15):
            return False
...
@sv.on_prefix('聊')
@sv.on_prefix('.c ')
async def gpt2_chat_force(bot, ev: CQEvent) -> None:
    if ev.group_id in TEST_GROUP:
        return
...
async def gpt2_chat(bot, ev: CQEvent) -> None:
    ...
    elif GPT2_loss < 3:  # 其他群: 满足概率 + loss够小
        await bot.send(ev, GPT2_msg)
