...
def set_interact_args():
    ...
    parser.add_argument('--batch_size', type=int, default=10, help='批量生成response，然后经过MMI模型进行筛选')
...
class RepeaterResult(object):
    def __init__(self, code = 2, msg = "", loss =0):
        self.code = code
        self.msg = msg
        self.loss = loss
    def toJSON(self):
...
def reply():
    ...
    out = generate_response(input_msg)  #(msg, loss)
    loss = out[1]
    msg_str = str(out[0])
    return RepeaterResult(0, msg_str, loss).toJSON()
...
def generate_response(input: str) -> (str, int):
    """输入string, 输出回复和对应的loss"""
    ...
    while True:
        try:
            ...
            top10_responses = []    # 可读response: ''.join(tokenizer.convert_ids_to_tokens(response))
                                    # 每个response包含 (response, loss)
            ...
            for response in candidate_responses:
                ...
                top10_responses.append((response, loss))
                samples_file.write("{} loss:{}\n".format("".join(text), loss))

            top10_responses.sort(key=lambda tup: tup[1])
            if  (top10_responses[1][1] - top10_responses[0][1] > 0.2) and top10_responses[0][1] > 3:
                best_response = top10_responses[0][0]
                best_loss = top10_responses[0][1]
            else:
                best_response = top10_responses[1][0]
                best_loss = top10_responses[1][1]

            if args.debug:
                for response in top10_responses:
                    text = tokenizer.convert_ids_to_tokens(response[0])
                    print("{} loss:{}".format("".join(text), response[1]))
            ...
            return_string = ''.join(tokenizer.convert_ids_to_tokens(best_response))
            return (return_string, best_loss)
